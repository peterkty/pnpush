...
def vizBlock(pose, frame_id, color, scale):
    global vizpub
    marker = createCubeMarker( 
                 offset=tuple(pose[0:3]), rgba=color,
                 orientation=tuple(pose[3:7]), frame_id=frame_id, scale = scale, marker_id = 1)
...
def main(argv):
    ...
    while not rospy.is_shutdown():
        vizBlock([surface_data['width']/2+0.150, 0, +surface_data['thickness']/2,0,0,0,1], frame_id, surface_data['color'], [surface_data['width'], surface_data['width'], surface_data['thickness']])
